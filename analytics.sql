...
/* ---------------------------------------------------------
   2) Top authors by number of books read.
   Топ-авторы по количеству прочитанных книг.
   --------------------------------------------------------- */
SELECT
  author,
  COUNT(*) AS books
...
/* ---------------------------------------------------------
   3) Reader activity (books & pages per reader).
...
FROM bookinsert
GROUP BY reader
ORDER BY pages_sum DESC, books DESC, reader;

/* ---------------------------------------------------------
   4) Page-count buckets (distribution).
...
/* ---------------------------------------------------------
   5) Genre counts (string-based; genre stored as CSV)
...
SELECT COUNT(*) AS with_any_genre
FROM bookinsert
WHERE COALESCE(TRIM(genre), '') <> '';

-- One row per configured genre (Fantasy, Romance, Thriller, Science fiction, Horror, Other)
SELECT
  'Fantasy'          AS genre, COUNT(*) AS books
FROM bookinsert
WHERE FIND_IN_SET('Fantasy', REPLACE(genre, ' ', '')) > 0
UNION ALL
SELECT 'Romance',    COUNT(*) FROM bookinsert WHERE FIND_IN_SET('Romance', REPLACE(genre, ' ', '')) > 0
UNION ALL
SELECT 'Thriller',   COUNT(*) FROM bookinsert WHERE FIND_IN_SET('Thriller', REPLACE(genre, ' ', '')) > 0
UNION ALL
SELECT 'Sciencefiction', COUNT(*) FROM bookinsert WHERE FIND_IN_SET('Sciencefiction', REPLACE(REPLACE(genre, ' ', ''), '-', '')) > 0
UNION ALL
SELECT 'Horror',     COUNT(*) FROM bookinsert WHERE FIND_IN_SET('Horror', REPLACE(genre, ' ', '')) > 0
UNION ALL
SELECT 'Other',      COUNT(*) FROM bookinsert WHERE FIND_IN_SET('Other', REPLACE(genre, ' ', '')) > 0;
...
/* ---------------------------------------------------------
   6) Recently finished books (last 10 by finished_date).
   Недавно завершённые книги (последние 10 по полю finished_date).
   --------------------------------------------------------- */
SELECT
  reader, title, author, genre, pages, finished_date
FROM bookinsert
WHERE finished_date IS NOT NULL
...
/* ---------------------------------------------------------
   7) Most recent additions (last 10 by added_at).
   Самые последние добавления (последние 10 по полю added_at).
   --------------------------------------------------------- */
SELECT
  reader, title, author, genre, pages, finished_date, added_at
FROM bookinsert
ORDER BY added_at DESC, id DESC
LIMIT 10;

/* ---------------------------------------------------------
   8) Duplicate checks (should be empty if UNIQUE(title_norm), UNIQUE(reader_norm) exist).
...
-- Possible duplicates by reader (case-insensitive)
SELECT LOWER(TRIM(reader)) AS r, COUNT(*) AS c
FROM bookinsert
GROUP BY r
HAVING c > 1;

/* ---------------------------------------------------------
   9) Views for quick reporting.
...
/* ---------------------------------------------------------
   10) Examples of using the views.
   Примеры использования представлений.
   --------------------------------------------------------- */
SELECT * FROM v_monthly_activity ORDER BY ym;
SELECT * FROM v_reader_totals ORDER BY pages_sum DESC, books DESC;